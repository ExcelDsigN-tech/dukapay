...
| `KYC_ENFORCEMENT_ENABLED` | — | ✓ | ✓ | `false` | Enable KYC/AML enforcement on borrower onboarding. **Intended default: `false` in dev only; must be `true` in staging and production** (enforced by `validateProductionComplianceFlags` + CI `check-prod-flags`). | `backend/src/services/complianceService.ts` |
...
| `AUDIT_ANCHOR_ENABLED` | — | ✓ | ✓ | `false` | Enable hourly tamper-evident audit anchoring. **Intended default: `false` in dev only; must be `true` in staging and production** (enforced by `validateProductionComplianceFlags` + CI `check-prod-flags`). | `backend/src/services/auditMerkleService.ts`, `backend/src/cron/auditAnchorJob.ts` |
