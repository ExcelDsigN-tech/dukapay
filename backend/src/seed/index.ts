...
const CONTRACT_ID = 'CDDUMMYDUKAPAYCONTRACT0000000000000000000000000000000000';
  ...
  for (const remittance of remittanceHistorySeeds) {
    ...
    if ((existing.rowCount ?? 0) > 0) {
      await query(
        `UPDATE remittance_history
         ...
         WHERE user_id = $1 AND month = $2`,
        [remittance.userId, remittance.month, remittance.amount, remittance.status],
      );
      continue;
    }
    ...
    await query(
      `INSERT INTO remittance_history (user_id, amount, month, status, created_at)
       VALUES ($1, $2, $3, $4, $5)`,
      [remittance.userId, remittance.amount, remittance.month, remittance.status, NOW],
    );
  }
};
...
const seedLoanHistory = async () => {
  ...
  for (const loan of loanHistorySeeds) {
    const existing = await query(`SELECT id FROM loan_history WHERE loan_id = $1`, [loan.loanId]);
...
const seedIndexerState = async () => {
  ...
  const existing = await query(`SELECT id FROM indexer_state ORDER BY id DESC LIMIT 1`);
