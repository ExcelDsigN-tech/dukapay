...
jest.unstable_mockModule('../db/connection.js', () => ({
  ...
  withTransaction: jest.fn(
    async (fn: (client: { query: typeof mockQuery; release: () => void }) => Promise<unknown>) => {
      ...
      const mockClient = {
        query: jest.fn(async (sql: string, params?: unknown[]) => mockQuery(sql, params ?? [])),
        release: jest.fn(),
      };
...
function makeRawEvent(params: {
  ...
  switch (params.type) {
    ...
    case 'LoanApproved':
      return {
        ...base,
        topic: [scSymbol('LoanApproved'), scU32(params.loanId ?? 1), scAddress(borrower)],
...
function makeAliasedEvent(params: {
  ...
  if (params.rawType === 'GovProp' || params.rawType === 'GovAppr' || params.rawType === 'GovFin') {
    return {
      ...base,
      topic: [scSymbol(params.rawType), scAddress(borrower)],
      value: scU32(1),
...
describe('EventIndexer', () => {
  ...
  it('quarantines parse failures and emits growth alert logs', async () => {
    ...
    expect(
      mockQuery.mock.calls.some(([sql]) => String(sql).includes('INSERT INTO quarantine_events')),
    ).toBe(true);
